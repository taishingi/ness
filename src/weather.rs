...
pub async fn show_weather() {
    ...
    let latitude: f64 = std::env::var("NESS_LATITUDE")
        .expect("failed to found latitude")
        .parse::<f64>()
        .expect("");
    let longitude: f64 = std::env::var("NESS_LONGITUDE")
        .expect("failed to found longitude")
        .parse::<f64>()
        .expect("");
    // Location
    ...
    for x in res.daily.iter() {
        for y in x.iter() {
            Notification::new()
                .summary(format!("Weather for {}", y.date).as_str())
                .body(
                    format!(
                        "Temperature max :  {} {}",
                        y.values["temperature_2m_max"].value,
                        y.values["temperature_2m_max"].unit.clone().expect("")
                    )
                    .as_str(),
                )
                .show()
